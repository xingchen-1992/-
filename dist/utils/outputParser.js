...
export function parseCodexOutput(rawOutput) {
    ...
    if (!response && thinking) {
        ...
        const substantiveLines = thinkingLines.filter(line => line.trim().length > 10 &&
            !line.includes('任务开始') &&
            !line.includes('任务完成') &&
            !line.startsWith('['));
        if (substantiveLines.length > 0) {
            ...
            // 保留非实质性的thinking内容
            thinking = thinkingLines.filter(line => line.includes('任务开始') ||
                line.includes('任务完成') ||
...
export function formatCodexResponse(output, includeThinking = true) {
    let formatted = '';
