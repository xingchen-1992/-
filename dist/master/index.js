...
export class 牛马Master {
     ...
     */
    async analyzeAndDecompose(claudePrompt) {
     ...
     * 基于任务类型智能生成子任务
     */
    generateSmartSubtasks(prompt, analysis) {
        const { suggestedTasks, taskType } = analysis;
        const subtasks = [];
        if (taskType === 'ui_state' && prompt.includes('按钮') && prompt.includes('状态')) {
            // UI状态管理问题的专门分解
            subtasks.push({ id: 'task_1', prompt: `分析UI按钮组件的实现和状态管理机制：${prompt}`, index: 1 }, { id: 'task_2', prompt: `分析按钮状态更新的事件处理和回调机制：${prompt}`, index: 2 }, { id: 'task_3', prompt: `分析音频播放完成后的状态同步逻辑：${prompt}`, index: 3 });
            if (suggestedTasks >= 4) {
            ...
            // 音频播放相关问题分解
            subtasks.push({ id: 'task_1', prompt: `分析音频播放管理器的实现：${prompt}`, index: 1 }, { id: 'task_2', prompt: `分析音频播放完成事件的处理：${prompt}`, index: 2 }, { id: 'task_3', prompt: `分析UI与音频状态的同步机制：${prompt}`, index: 3 });
        }
        else {
            // 通用任务分解
    ...
    mergeResults(results) {
        ...
        // 添加统计信息
        const successCount = results.filter((r) => !r.response.includes('执行失败')).length;
     ...
     * 步骤4: 基于整合结果生成执行计划
     */
    async generateExecutionPlan(originalPrompt, mergedResults) {
