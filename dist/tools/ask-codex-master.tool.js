import { z } from 'zod';
import { 牛马Master } from '../master/index.js';
 ...
 * 5. 返回给Claude可直接执行的方案
 */
export const askCodexMasterTool = {
    ...
    async execute(args, onProgress) {
        const { prompt, includeAnalysis = true } = args;
        // 验证必要参数
...
🚀 开始任务分解和并行执行...`);
            ...
            onProgress?.('🔍 任务分析和分解中...');
            // 执行完整的Master流程
            const executionPlan = await master.execute(prompt);
 ...
 * 格式化Master响应给Claude
 */
function formatMasterResponse(plan, includeAnalysis, progressFile, monitorScript) {
...
//# sourceMappingURL=ask-codex-master.tool.js.map