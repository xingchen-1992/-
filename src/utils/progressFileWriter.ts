...
export class ProgressFileWriter {
  ...
  constructor(logDir: string = 'H:\\A_test\\牛马\\logs') {
    // 确保日志目录存在
   ...
   * 更新进度数据
   */
  async updateProgress(progressData: Partial<ProgressData>): Promise<void> {
    if (!this.isActive) {
      return;
    }
   ...
   * 写入进度数据到文件（覆盖写入）
   */
  private writeProgress(data: ProgressData): void {
    try {
...
// 单例模式：全局进度文件写入器实例
let progressFileWriterInstance: ProgressFileWriter | null = null;

/**
 * 获取全局进度文件写入器实例
 */
export function getProgressFileWriter(): ProgressFileWriter {
 ...
 * 清理全局实例（用于测试或重置）
 */
export function resetProgressFileWriter(): void {
  if (progressFileWriterInstance) {
